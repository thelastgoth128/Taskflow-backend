...
export class TeamsService {
 ...
 async create(createTeamDto: CreateTeamDto,@Req() req:Request) {
  ...
  async findAll(@Req() req:Request) {
    ...
    const teamId = member.map(members=>members.teamid)
    const teams = await this.team.find({where : {teamid: In(teamId)},relations:[
    'teamleader','teammembers.userid','teammembers.teamid']})
